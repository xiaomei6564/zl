import { get, getDictMap, enumerated } from '@/api/dictDetail'

export default {
  data() {
    return {
      dicts: [], dictMap: {}, hanaDictMap: {}
    }
  },
  methods: {
    async getDict(name) {
    ...
    // hana
    async getHanaDictMap(names) {
      // 优先放入到dictMap中，避免页面加载时 undefined
      const arr = names.split(',')
      for (let i = 0; i < arr.length; i++) {
        this.hanaDictMap[arr[i]] = []
      }
      return new Promise((resolve, reject) => {
        enumerated(names).then(res => {
          this.hanaDictMap = res
          resolve(res)
        }).catch(err => {
          reject(err)
        })
